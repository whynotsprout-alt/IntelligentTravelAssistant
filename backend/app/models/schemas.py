...
class TripRequest(BaseModel):
    ...
    start_date: dt.date = Field(..., description="开始日期 YYYY-MM-DD", example="2025-06-01")
    end_date: dt.date = Field(..., description="结束日期 YYYY-MM-DD", example="2025-06-03")
    ...
    def validate_travel_days(self) -> "TripRequest":
        expected_days = (self.end_date - self.start_date).days + 1
        if expected_days != self.travel_days:
            raise ValueError(
                f"travel_days({self.travel_days}) must equal date range length ({expected_days})"
            )
        return self
...
class DayPlan(BaseModel):
    """单日行程"""
    date: dt.date = Field(..., description="日期 YYYY-MM-DD")
